...
Measurer 是一個 PySide6 desktop GUI tool，用來量測半導體 MOM 結構 STEM ZC 影像中的 metal 尺寸與 spacing。工具定位是給工程師逐張檢查 ROI、執行量測、確認 overlay，最後批次匯出結果。
...
- Original / Overlay / Box Plot / Debug preview。
- GUI box plot 顯示 group 分布。
- Export 時由使用者指定 output folder。
- 匯出 annotated images、debug images、xlsx。
...
右側 Image Workspace:
- view mode controls
- image preview / overlay / box plot / debug
```
...
- 使用者指定 output folder。
- 批次匯出所有已量測圖片。
- 若 output folder 已有舊結果，預設覆蓋，但必須先跳確認 dialog。
...
GUI 不需要顯示 scale source。Excel Details 可以記錄 scale source 方便追查。
...
- Overlay 不顯示 ROI。
- exported annotated image 不顯示 ROI。
- Debug view 可以顯示 ROI。
...
- Debug / Details 可記錄 all candidates touched analysis boundary。
...
- Original / Overlay / Box Plot 是主要切換。
...
- 失敗 value 留空。
- reason、座標、value_px、scale、ROI 放 Details。
