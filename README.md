...
`assets/icons/measurer.png` 是同一個 icon 的 PNG 版本，方便 README、installer 或其他包裝流程預覽使用。這個 icon 使用 Aligner / Denoiser / Measurer 共用的深色圓角 icon 風格。Windows 打包流程必須把 icon 參數指向 `assets/icons/measurer.ico`，確保打包後的 exe / shortcut 顯示 Measurer 品牌 icon。
