...
- file queue 支援選取一列或多列後用 Set Group 套用 group name；左側只顯示 `File`、`Group`、`Status` 三欄，Status 以兩行整合 ROI / Measure / Export，不需要水平捲動才看狀態。
...
- Box Plot preview 已可從 GUI 切換，會依 Group 與 measurement type 彙整 successful final measurements，使用 canvas 依目前顯示尺寸繪製 raw points with jitter 與 summary/status panel；x-axis 先依 measurement type 分群，再在群內並排各 Group 方便比較。
- Box Plot preview 提供 TCD / BCD / Height / Horizontal Space / Vertical Space checkbox，可只顯示勾選的 measurement types；切換 checkbox 會用現有 results 立即刷新，不需要重測。
...
- Box Plot 使用 display-resolution drawing，避免固定 pixmap 放大造成字體模糊；x-axis 以兩層 label 顯示，Group label 對齊各 bucket，measurement type label 對齊該 type 的 group cluster 中央。
- 8 px radius。
- Primary action 使用藍色。
...
- TCD、BCD、Height、Horizontal Space、Vertical Space checkbox 預設全選；取消勾選只影響 GUI Box Plot preview，不影響 Result View / Export / Excel。
